...
## 13. UI/UX Refinements (Session 1)

These refinements supersede or extend the relevant sections above. They reflect the iterative feedback applied during the first build/polish cycle and should be honored by any restart-from-scratch implementation.

### 13.1 Branding & Home

- Home heading must be bilingual: top line "Welcome to eFancy!" and a Chinese subtitle "欢迎来到时尚坊市！".
- The four module tiles (eSpectacles, eGroceries, eFreshes, eServices) must be reorderable by the user:
  - Desktop: HTML5 drag-and-drop reorder.
  - Mobile: touch drag-and-drop with `elementFromPoint` hit-testing, a 6 px movement threshold, and `touch-action: none` on the dragged tile.
  - The chosen order persists per device in `localStorage` under the key `efancy_home_order` and is restored on next load.

### 13.2 Choose Frame Page

- A single click on a frame card must do BOTH actions simultaneously:
  1. Select the frame (highlight the card and enable the top "Next" button).
  2. Open the enlarged-image popup.
- The popup supports left/right swipe across multiple frame images and has a × close control at the top-right.

### 13.3 Eyesight Data Page (Page B)

- On entry (including when the user navigates back to this page), BOTH radio buttons must be unselected — neither "No Eyesight Data" nor "Having Eyesight Data" is pre-checked. The shop-search panel only appears after "No Eyesight Data" is explicitly chosen.
- Selecting "Having Eyesight Data" must NOT reveal an inline form on this page. Instead, it must navigate to a dedicated full-page eyesight-entry screen (see 13.4).
- "No Eyesight Data" flow:
  - Show partner-shop search and the configurable Eyesight Checkup fee (default $20, key `checkup_fee`).
  - Selected shop is persisted into the order draft (`draft.checkupShop`) so it survives back-navigation.
  - Tapping the Pay button creates a checkup order and navigates to the Payment screen (no auto-pay).

### 13.4 Manual Eyesight Entry Page

- Dedicated route, reachable in two ways:
  1. Pre-order, from the Eyesight Choice page (no order id).
  2. Post-order, from an existing Eyesight Checkup order's detail page via the green "I Have My Eyesight Data Now" button (carries the order id).
- When entered with an order id, show an informational note: the existing Eyesight Checkup order #N is not affected and may be cancelled separately.
- Section headings must be title-cased: "Left Eye" and "Right Eye".
- Inputs: SPH, CYL, Axis, ADD (per eye) plus PD. ADD is optional; all others are required.
- All numeric inputs must:
  - Accept signed real numbers, e.g. `-2.50`, `+1.25`, `0`, `.5`. Allowed characters are digits, one `.`, and a leading `-` or `+`.
  - Reject every other keystroke and, on rejection, show a modal warning ("Invalid character. Only numbers are allowed. You may use `.`, `-`, or `+` for decimal and signed values.") with a × close button, an explicit Close button, and backdrop-tap dismissal.
  - Display blank (not "NaN") when the stored value is null/NaN.
  - Show helper placeholders such as `e.g. -2.50`, `0–180`, `optional`, `e.g. 62`.
- The Save button must always navigate to the Ordering page (regardless of whether a frame was chosen earlier; if needed, the Ordering page handles missing frame state).

### 13.5 Ordering Page (Page C)

- Above the chosen-frame block, render a prominent label `Frame Chosen:` in larger size, bold weight, solid black.
- The frame card itself stays inside its own card and includes a Change button that returns to Choose Frame while preserving lens selections.

### 13.6 Confirmation Page (Page E)

- Eyesight summary table must be column-aligned. Use a fixed table layout with `colgroup` widths so the values sit directly under their headers SPH / CYL / Axis / ADD.
- Row labels: `Left:` and `Right:` (replacing single-letter L/R), left-aligned. Numeric cells centered.
- Pupil distance line reads `Pupil Distance(PD): <value>` rendered in solid black (not the muted/grey style).

### 13.7 Payment Page (Page F)

- Top-left Back button styled at `fontSize: 1.5em`, navigates with browser history (`-1`).
- Show a per-method input form:
  - Credit/Debit Card: 4-group card-number formatter, expiry MM/YY, CVV, name on card. Card number is validated against the Luhn algorithm on blur; if invalid, show a warning modal ("Invalid card number") with × top-right and a Close button. The Pay action is blocked until the card is Luhn-valid.
  - PayLah! and PayNow: mobile-number field.
  - WeChat Pay: WeChat ID field.
  - AliPay: AliPay account field.
- Success screen for a checkup order replaces the generic Done button with a "Show QR" button that navigates to the Eyesight Checkup Pending screen.

### 13.8 Eyesight Checkup Pending Screen

- Heading: "Eyesight Checkup pending" with a Back button (1.5em) on the left.
- Body shows the order code, partner shop name, and a status badge whose displayed text is overridden by a helper: when `module === 'checkup'` and `status === 'Pending'`, render "Pending for Eyesight Checkup" instead of the raw `Pending` enum value. The underlying enum stays `Pending` so backend logic is unaffected.
- QR section: render a real QR image (size 168 px) using a QR library; below the image show the raw token in monospace, 1.5em, bold.
- A "Change Shop" button is visible while status is Pending or Paid. It opens a modal that lists partner shops with the current shop pre-selected and labelled "(current)"; the Change action is disabled while the picked shop equals the current shop. Confirming calls the backend partner-shop change endpoint and updates both `shop_id`, the meta JSON, and the checkup line-item label ("Eyesight Checkup at <shop>").

### 13.9 Order Detail Screen

- Header card shows: Status (with the status-label override from 13.8), Module (display name from a module-label helper — e.g. `checkup` renders as "Eyesight Checkup", and the four primary modules render as "eSpectacles", "eGroceries", "eFreshes", "eServices"), `Total Paid: $X.XX`, and optional Paid/Finalised/Cancelled timestamps.
- Partner-shop block uses the label `Partner Shop:` (title case) and includes a Change Shop button + modal for checkup orders in Pending/Paid status, plus an inline QR (size 154 px) while status is Pending.
- For a checkup order in Pending or Paid status, render a GREEN button labelled `I Have My Eyesight Data Now` immediately above the danger-coloured `Cancel order` button. Clicking it navigates to the Manual Eyesight Entry page (13.4) with the order id in the URL.

### 13.10 Orders List

- Each row shows the order code, the status badge (with the status-label override), the module display name (e.g. "Eyesight Checkup"), the total, and the created-at timestamp.

### 13.11 Terminology

- User-facing copy must use title case for the following terms wherever they appear as labels or buttons (URLs, enum values, and config keys keep their lowercase identifiers):
  - "Eyesight Checkup" (never "checkup" or "Checkup" alone in user-facing strings).
  - "Pending for Eyesight Checkup" as the displayed status for checkup orders awaiting shop upload.
  - "Total Paid" (not "Total") on order detail.
  - "Change Shop" (button) and "Partner Shop" (label).
  - "Left Eye" and "Right Eye" (section headings).
  - "Pupil Distance(PD):" (label).
  - "Frame Chosen:" (label above the chosen frame on Ordering).
  - "I Have My Eyesight Data Now" (button).

### 13.12 Reusable Components & Styling

- Provide a shared `NumInput` component (signed-real-number filter + NaN sanitization + invalid-character modal) used by both the Eyesight Choice and Manual Eyesight Entry pages.
- Provide `statusLabel(order)` and `moduleLabel(module)` helpers used by the Orders list, Order Detail, and Checkup Pending screens.
- Add a `.btn.success` class (green) alongside the existing `.btn`, `.btn.secondary`, `.btn.ghost`, `.btn.danger` variants.
- Maintain a mobile-first responsive layout (content max-width ~480 px on phones, ~720 px on desktop) with the top scalable tabs and the Cart / Orders / Admin / Me bottom bar; the Admin tab is only visible to `admin` and `super_admin` users.

### 13.13 Backend Reinforcement

- `PATCH /api/orders/:id/checkup/shop` allows changing a partner shop while the checkup order is Pending or Paid; the endpoint updates `shop_id`, refreshes `meta_json.shop_name`, and rewrites the corresponding `order_items` label to `Eyesight Checkup at <shop>`.
- All user-facing strings generated by the backend (line-item labels, notification text) use the title-cased terminology in 13.11.
- In non-production environments, the OTP endpoint additionally returns the generated code as `devCode` to ease testing, while still printing it to the server console.
